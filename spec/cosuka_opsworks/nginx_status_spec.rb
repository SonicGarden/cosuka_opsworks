  ...
  describe '#usage' do
    ...
    context 'worker_connections = 1024, active_connections = 10' do
      it '1%' do
        ...
        allow(status).to receive(:`).with("curl http://127.0.0.1/nginx_status | grep 'Active connections'").and_return('Active connections: 10')
    ...
    context 'worker_connections = 1024, active_connections = 100' do
      it '10%' do
        allow(status).to receive(:`).with('grep worker_connections /etc/nginx/nginx.conf').and_return('  worker_connections  1024;')
        allow(status).to receive(:`).with("curl http://127.0.0.1/nginx_status | grep 'Active connections'").and_return('Active connections: 100')
    ...
    context 'command error' do
      it 'raise error' do
        ...
        allow(status).to receive(:`).with("curl http://127.0.0.1/nginx_status | grep 'Active connections'").and_return('')
